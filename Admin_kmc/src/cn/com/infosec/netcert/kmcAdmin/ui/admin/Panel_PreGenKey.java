...
public class Panel_PreGenKey extends ApplicationWindow {
	...
	protected Control createContents(Composite parent) {
		...
		final Text txt_alg = new Text(container, SWT.BORDER | SWT.READ_ONLY);
		GridData gd_txt_alg = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		gd_txt_alg.widthHint = 112;
		txt_alg.setLayoutData(gd_txt_alg);
		...
		combo_length.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				pre_countKeys(combo_length.getText());
			}
		});
		...
		GridData gd_lb_cnt = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		gd_lb_cnt.widthHint = 119;
		lb_cnt.setLayoutData(gd_lb_cnt);

		Label lbl_refresh = new Label(container, SWT.NONE);
		GridData gd_lbl_refresh = new GridData(SWT.RIGHT);
		gd_lbl_refresh.widthHint = 73;
		gd_lbl_refresh.horizontalAlignment = SWT.RIGHT;
		lbl_refresh.setLayoutData(gd_lbl_currNum);
		lbl_refresh.setAlignment(SWT.RIGHT);
		...
		btn_genKey.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
					...
					p.setProperty("KEYALG", txt_alg.getText().trim());
					...
					try {
						...
						mb.setMessage(l.getString("Notice_preGenKeying"));
						mb.open();
	 ...
	 * @version 1.0
	 */
	private void pre_countKeys(String keySize) {
		Properties p = new Properties();
		p.setProperty("RANGE", "pre");
		p.setProperty("STATTYPE", keySize);
		try {
